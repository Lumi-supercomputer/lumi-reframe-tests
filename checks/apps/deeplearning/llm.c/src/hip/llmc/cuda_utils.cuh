...
DType dtype_of(__hip_bfloat16 * f) { return DType::BF16; }
...
template<class Float>
void global_sum_deterministic(float* result, const Float* values, int count, hipStream_t stream) {
...
int cudaMallocConditionallyManaged(void** out, size_t bytes, const char *file, int line) {
    // try to allocate
    hipError_t err = hipMalloc(out, bytes);
    if(err == hipErrorOutOfMemory) {
        // if we OOM, fallback to a managed allocation. slower but at least won't crash.
        hipGetLastError(); // reset the error before the next API call
        cudaCheck_(hipMallocManaged(out, bytes), file, line);
        cudaCheck_(hipMemAdvise(*out, bytes, hipMemAdviseSetPreferredLocation, hipCpuDeviceId), file, line);
        return 1;
    } else {
