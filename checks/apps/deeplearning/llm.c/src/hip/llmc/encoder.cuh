...
void encoder_backward(floatX* dwte, floatX* dwpe, floatX* scratch, // gpu outputs & scratch
                      ...
                      int B, int T, int C, unsigned int seed, hipStream_t stream) {
    //mNVTX_RANGE_FN();
    ...
    cudaCheck(hipMemcpyAsync(d_bucket_info, bucket_info, num_buckets * sizeof(int4), hipMemcpyHostToDevice, stream));
    cudaCheck(hipMemcpyAsync(d_workload_indices, workload_indices, total_items * sizeof(int), hipMemcpyHostToDevice, stream));
