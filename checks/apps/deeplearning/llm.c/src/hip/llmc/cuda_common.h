...
//m#include <nvtx3/nvToolsExt.h>
//m#include <nvtx3/nvToolsExtCudaRt.h>
#include <hip/hip_runtime_api.h>
//m#include <cuda_bf16.h>
...
inline void cudaCheck_(hipError_t error, const char *file, int line) {
  if (error != hipSuccess) {
...
// like hipFree, but checks for errors _and_ resets the pointer.
template<class T>
...
inline void device_to_file(FILE* dest, void* src, size_t num_bytes, size_t buffer_size, hipStream_t stream) {
    // allocate pinned buffer for faster, async transfer
    char* buffer_space;
    cudaCheck(hipHostMalloc(&buffer_space, 2*buffer_size));
    ...
    cudaCheck(hipMemcpyAsync(read_buffer, gpu_read_ptr, copy_amount, hipMemcpyDeviceToHost, stream));
    ...
    while(rest_bytes > 0) {
        ...
        cudaCheck(hipStreamSynchronize(stream));     // wait for both buffers to be ready.
    ...
    fwriteCheck(write_buffer, 1, write_buffer_size, dest);
    cudaCheck(hipHostFree(buffer_space));
}

// copy num_bytes from file src into device pointer dest, using double buffering running on the given stream.
inline void file_to_device(void* dest, FILE* src, size_t num_bytes, size_t buffer_size, hipStream_t stream) {
     // allocate pinned buffer for faster, async transfer
    ...
    char* buffer_space;
    cudaCheck(hipHostMalloc(&buffer_space, 2*buffer_size, hipHostMallocWriteCombined));
    // split allocation in two
    void* read_buffer = buffer_space;
    void* write_buffer = buffer_space + buffer_size;
    ...
    while(rest_bytes > 0) {
        // initiate next read
        copy_amount = std::min(buffer_size, rest_bytes);
        cudaCheck(hipMemcpyAsync(gpu_write_ptr, write_buffer, write_buffer_size, hipMemcpyHostToDevice, stream));
