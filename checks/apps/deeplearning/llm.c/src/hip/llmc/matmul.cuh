...
// Wrapper around hipblasLtMatmul that is meant to support everything we need in llm.c
// https://docs.nvidia.com/cuda/cublas/#cublasltmatmul
void matmul_cublaslt(floatX* d, const floatX* a, const floatX* b, const floatX* bias,
                     int m, int n, int k, hipStream_t stream=0, bool transA=true, bool transB=false,
                     ...
                     bool accumulate=false, floatX* pre_gelu=NULL, bool backward=false)
{
    ...
    cublasCheck(hipblasLtMatmulDescCreate(&operationDesc, cublas_compute, HIP_R_32F));
    ...
    cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_TRANSA, (transA)  ? &opTranspose : &opNoTranspose,   sizeof(opTranspose)));
    cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_TRANSB, (transB) ? &opTranspose   : &opNoTranspose, sizeof(opNoTranspose)));

    // define matrix layouts
    hipblasLtMatrixLayout_t ALayout;
    hipblasLtMatrixLayout_t BLayout;
    hipblasLtMatrixLayout_t DLayout;
    hipblasLtMatrixLayout_t CLayout;
    if (transA) {
        cublasCheck(hipblasLtMatrixLayoutCreate(&ALayout, CUBLAS_LOWP, k, m, k));
    } else {
        cublasCheck(hipblasLtMatrixLayoutCreate(&ALayout, CUBLAS_LOWP, m, k, m));
    }
    if (transB) {
        cublasCheck(hipblasLtMatrixLayoutCreate(&BLayout, CUBLAS_LOWP, n, k, n));
    } else {
        cublasCheck(hipblasLtMatrixLayoutCreate(&BLayout, CUBLAS_LOWP, k, n, k));
    }
    // cuBLASLt requires C in FP8 mode to be BF16 or FP32... (sigh)
    cublasCheck(hipblasLtMatrixLayoutCreate(&CLayout, (sizeof(floatX) == 1) ? HIP_R_16BF : CUBLAS_LOWP, m, n, m));
    cublasCheck(hipblasLtMatrixLayoutCreate(&DLayout, CUBLAS_LOWP, m, n, m));

    // Strided Batched GEMM (used for non-flash attention, equivalent to hipblasGemmStridedBatchedEx_v2)
    if (batch_count) {
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(ALayout, HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(BLayout, HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(CLayout, HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(DLayout, HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, &batch_count, sizeof(batch_count)));

        cublasCheck(hipblasLtMatrixLayoutSetAttribute(ALayout, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &strideA, sizeof(strideA)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(BLayout, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &strideB, sizeof(strideB)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(CLayout, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &strideOut, sizeof(strideOut)));
        cublasCheck(hipblasLtMatrixLayoutSetAttribute(DLayout, HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &strideOut, sizeof(strideOut)));
    }

    // create a preference handle with specified max workspace
    cublasCheck(hipblasLtMatmulPreferenceCreate(&preference));
    cublasCheck(hipblasLtMatmulPreferenceSetAttribute(preference, HIPBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES,
    ...
    if (has_gelu) {
        ...
        cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_EPILOGUE_AUX_LD, &gelu_ld, sizeof(gelu_ld)));
        cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_EPILOGUE_AUX_POINTER, &pre_gelu, sizeof(pre_gelu)));
        if (backward) {
            assert(!has_bias); // we shouldn't have any backward matmuls that use both GELU and bias
            epilogue = HIPBLASLT_EPILOGUE_DGELU;
        } else {
            epilogue = has_bias ? HIPBLASLT_EPILOGUE_GELU_AUX_BIAS : HIPBLASLT_EPILOGUE_GELU_AUX;
        }
    } else if(has_bias){
        epilogue = backward ? HIPBLASLT_EPILOGUE_BGRADB : HIPBLASLT_EPILOGUE_BIAS;
    } else {
        epilogue = HIPBLASLT_EPILOGUE_DEFAULT;
    }
    cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_EPILOGUE, &epilogue, sizeof(epilogue)));

    if (has_bias) {
        // cuBLASLt requires bias in FP8 mode to be BF16... (sigh)
        hipDataType bias_data_type = (sizeof(floatX) == 1) ? HIP_R_16BF : CUBLAS_LOWP; // force BF16 bias for FP8 mode
        cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_BIAS_DATA_TYPE, &bias_data_type, sizeof(bias_data_type)));
        cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, HIPBLASLT_MATMUL_DESC_BIAS_POINTER, &bias, sizeof(bias)));
    }
/* mszpindler
    // set scale type to FP32 (needs to be FP16 if and only if using HIPBLAS_COMPUTE_16F, so it's FP32 even for FP8!)
    hipDataType scale_type = HIP_R_32F;
    cublasCheck(hipblasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_SCALE_TYPE, &scale_type, sizeof(scale_type)));
*/
    // find a suitable algorithm (cached internally so shouldn't take much CPU time in practice)
    hipblasLtMatmulAlgoGetHeuristic(cublaslt_handle, operationDesc, ALayout, BLayout, CLayout, DLayout,
    ...
    cublasCheck(hipblasLtMatrixLayoutDestroy(DLayout));
