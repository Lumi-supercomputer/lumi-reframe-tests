...
typedef struct {
    ...
    hipStream_t nccl_stream;   // CUDA Stream to perform NCCL operations.
    hipEvent_t compute_nccl_sync; // Event used to synchronize NCCL with the compute
...
MultiGpuConfig multi_gpu_config_init(int num_processes, int process_rank, int gpus_per_node, char* server_ip, char* fs_path, char* init_method) {
    ...
    cudaCheck(hipStreamCreate(&result.nccl_stream));
    // event without timing for maximum performance
    cudaCheck(hipEventCreateWithFlags(&result.compute_nccl_sync, hipEventDisableTiming));
    nvtxNameCudaStreamA(result.nccl_stream, "nccl stream");
    nvtxNameCudaEventA(result.compute_nccl_sync, "nccl compute sync");
    cudaCheck(hipMallocManaged(&result.unified_buffer, sizeof(float)));
    return result;
#else
...
void multi_gpu_config_free(MultiGpuConfig* config) {
    ...
    cudaCheck(hipStreamDestroy(config->nccl_stream));
    cudaCheck(hipEventDestroy(config->compute_nccl_sync));
    cudaCheck(hipFree(config->unified_buffer));
    #ifdef USE_MPI
...
void multi_gpu_async_reduce_gradient(
        ...
        MultiGpuConfig* config, hipStream_t compute_stream) {
    if (config->num_processes == 1) {
    ...
    cudaCheck(hipEventRecord(config->compute_nccl_sync, compute_stream));
    cudaCheck(hipStreamWaitEvent(config->nccl_stream, config->compute_nccl_sync));
